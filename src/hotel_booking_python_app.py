    ...
    <section class="view active" id="rooms">
      ...
      <div class="page-heading">
        ...
        <form class="sort-control" method="get" action="/rooms">
          {hidden_context}
          <label><span>Room type</span><select name="type">
...
def booking_page(room, logged_in, params=None):
    ...
    <section class="view active" id="booking">{feedback}
      <div class="booking-layout">
        <form class="wide-form" method="post" action="/payment">
...
def payment_page(room, logged_in, form_data):
    room_code = first_value(form_data, "room", "")
    check_in = clean_query_value(first_value(form_data, "checkin", ""))
    check_out = clean_query_value(first_value(form_data, "checkout", ""))
    guests = clean_query_value(first_value(form_data, "guests", "1"))
    guest_data = {
        "guest": first_value(form_data, "guest"),
        "passport": first_value(form_data, "passport"),
        "phone": first_value(form_data, "phone"),
    }
    ...
    can_book, _availability_label = room_bookability_for_dates(
        room,
        {"checkin": [check_in], "checkout": [check_out], "guests": [str(guest_count)]},
    )
    if not can_book:
        raise ValueError("Room is not available for the selected stay.")
    total = room["price"] * nights
    hidden_fields = "".join(
    ...
    body = f"""
    <section class="view active" id="payment">
      <div class="payment-page-card">
        ...
        <div class="payment-details">
          <div><span>Room</span><strong>{esc(room['type'])} Room {esc(room['number'])}</strong></div>
          <div><span>Check-in</span><strong>{esc(check_in)}</strong></div>
          <div><span>Check-out</span><strong>{esc(check_out)}</strong></div>
          <div><span>Guests</span><strong>{esc(guest_count)}</strong></div>
          <div><span>Amount to pay</span><strong>{money(total)}</strong></div>
        </div>
        <form class="payment-result-form" method="post" action="/confirmation">
          {hidden_fields}
    ...
    """
    return page("Online Payment - Lucky Hotel", body, logged_in, "rooms")
...
class HotelHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/login":
            ...
            room = room_by_id(room_code)
            if room is None:
                self.send_html(room_missing_page(True))
                return
            try:
                self.send_html(payment_page(room, True, data))
            except Exception as error:
                self.send_html(
                    payment_message_page(
                        "Payment Error",
                        "Payment Not Started",
                        str(error) or "Payment information could not be prepared.",
                        room_code,
                        first_value(data, "checkin", ""),
                        first_value(data, "checkout", ""),
                        first_value(data, "guests", "1"),
                    )
                )
